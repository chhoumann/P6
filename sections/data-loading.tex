...
In section \ref{sec:data loading and preprocessing}, we start by presenting the data loading and preprocessing phase of the system architecture, and we also motivate our in the transformer model.
Then, in sections \ref{sec:self-attention mechanism} and \ref{sec:multi-head attention}, we describe preliminary theory about the transformer model--the self-attention mechanism and multi-head attention. 
Afterwards, in section \ref{sec:time2vec} we introduce the method we use for embedding temporal information in the transformer model. 
In section \ref{sec:training}, we describe our approach for training the models and how we serialize the trained models into the Python \texttt{pickle} format.
