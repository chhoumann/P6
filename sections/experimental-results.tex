...
\begin{itemize}
    \item \textbf{Austin}\cite{HistoricalHourlyWeather}: The raw dataset over weather features in Austin, Texas covers a 4 year period from 2013 to 2017 with data logged once per day in the period.
...
\begin{itemize}
    \item \textbf{ARIMA}\cite{ARIMA-book}: A generalisation of the ARMA model that can be fitted to data in order to predict future steps.
    \item \textbf{GRU}\cite{GRU-paper}: A RNN model similar to LSTM but with a forget-gate added instead of an output gate.
    \item \textbf{LinReg}: A standard linear regression model.
    \item \textbf{LSTM}\cite{LSTM-paper}: A model modifying the RNN memory system in order to combat the vanishing gradient problem.
    ...
    \item \textbf{Transformer}\cite{AttentionIsAllYouNeed}: A state of art model used in NLP, modified to work for time series forecasting.
\end{itemize}
